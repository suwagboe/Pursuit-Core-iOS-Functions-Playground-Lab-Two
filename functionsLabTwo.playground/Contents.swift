...
func average (of: [Double]) -> Double { // cant call the function inside of the function
   // var avr = 0.0
    var sum = 0.0
...
// Your function here
// need help
//func frequencyDictionary(of string: String ) -> Dictionary {
//    var numOfOccurancesDict = [Character: Int]
//
//    for char in string{
//        numOfOccurancesDict
//    }
...
// using module in if statments to prove if it true or false
    var theWord = ""
    for num in 1...n {
        if num % 3 == 0{
            theWord = "fizz"
...
func value(isGreaterThanAverageOf two: Double, if one:[Double]) -> Bool {
    ...
    if two > average {
        // comparing the average of one to the value of isGreaterThanAverageOf
        condition = true
    } else {
        condition = false
    }
...
func secondSmallestValue(in smallNum: [Int]) -> Int{
       let sortedIntArray = smallNum.sorted()
